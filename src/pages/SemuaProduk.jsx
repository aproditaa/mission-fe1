        ...
        <div className="flex flex-col md:flex-row gap-8">
            ...
            <div className="bg-white p-6 rounded-[15px] border border-[#E5E5E5] shadow-sm">
              <div className="flex justify-between items-center mb-6">
                ...
                <button 
                  ...
                  className="text-[#FF4D4D] text-[14px] font-semibold hover:underline"
                >
                  Reset
                </button>
              </div>
                  ...
                  <div className="flex flex-col gap-3 ml-7">
                    {["Kurang dari 4 Jam", "4 - 8 Jam", "Lebih dari 8 Jam"].map((range) => (
                      <label key={range} className="flex items-center gap-3 cursor-pointer">
                        <input 
                          type="checkbox" 
                          checked={selectedDuration.includes(range)}
                          onChange={() => {
                            if (selectedDuration.includes(range)) {
                              setSelectedDuration(selectedDuration.filter(d => d !== range));
                            } else {
                              setSelectedDuration([...selectedDuration, range]);
                            }
                          }}
                          className="w-4 h-4 rounded accent-[#3ECF4C]" 
                        />
          ...
          <div className="flex-1">
            <div className="flex flex-col md:flex-row gap-4 mb-8">
              <select 
                className="md:w-[200px] h-[45px] px-4 border border-[#E5E5E5] rounded-lg bg-white outline-none text-sm cursor-pointer"
                value={sortBy}
              ...
              <div className="relative flex-1">
                <input 
                  ...
                  className="w-full h-[45px] pl-4 pr-10 border border-[#E5E5E5] rounded-lg focus:ring-1 focus:ring-[#3ECF4C] outline-none"
                />
